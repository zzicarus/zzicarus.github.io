...
<img src="images/custom-image-4.png" alt="alt text" style="zoom:50%;" />
...
??? example "example"
    <img src="images/custom-image-7.png" alt="alt text" style="zoom:50%;" />
...
!!! note "理解cache中的各个位"
    ...
    `Offset` 用于去在data中寻址。注意寻址方式，一般是字节寻址。比如，一个 4-word blocks，就需要 index 先去寻找block，再由offset分别确定word和byte，故需要4bit
    
    所以cache实际的size是 data size of a block + Tag + Valid
...
- **Size**
    ...
    - Virtual Address - Page Offset  =  Virtual Number
